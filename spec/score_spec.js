const {outputGrade} = require('../src/score');
...
describe('Grade output function', () => {
    ...
    it('should throw an error if the number is out of range', () => {
        expect(outputGrade(-1)).toBe('input must not be null / must be a number');
    });

    it('should throw an error if the number is out of range', () => {
        expect(outputGrade(1.3)).toBe('input must not be null / must be a number');
    });
});